...
public class SuperTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link SuperTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link SuperTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    public static final int GRAVITY_CENTER = 1;
    public static final int INDICATOR_WIDTH_MATCH = 0;
    public static final int INDICATOR_WIDTH_WRAP = 1;
    public static final int INDICATOR_WIDTH_EXACTLY = 1 << 1;
    public static final int INDICATOR_HEIGHT_MATCH = 0;
    public static final int INDICATOR_HEIGHT_WRAP = 1;
    public static final int INDICATOR_HEIGHT_EXACTLY = 1 << 1;
    private static final int DEFAULT_HEIGHT_WITH_TEXT_ICON = 48; // dps
    ...
    private int DEFAULT_GAP_TEXT_ICON = 8; // dps
    ...
    public SuperTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SuperTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // Add the TabStrip
        mTabStrip = createSlidingTabStrip(context);
        super.addView(mTabStrip, 0, new LayoutParams(
                LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));

        TypedArray t = context.obtainStyledAttributes(attrs, R.styleable.SuperTabLayout);
        //------add start------
        int widthMode = t.getInt(R.styleable.SuperTabLayout_indicatorWidthMode,
                INDICATOR_WIDTH_MATCH);
        mTabStrip.setIndicatorWidthMode(widthMode);
        int indicatorWidth = t.getDimensionPixelSize(R.styleable.SuperTabLayout_indicatorWidth, 0);
        mTabStrip.setIndicatorWidth(indicatorWidth);

        int heightMode = t.getInt(R.styleable.SuperTabLayout_indicatorHeightMode,
                INDICATOR_HEIGHT_EXACTLY);
        mTabStrip.setIndicatorHeightMode(heightMode);

        mTabStrip.setCornerRadius(t.getDimensionPixelSize(R.styleable.SuperTabLayout_indicatorRadius, 0));

        mTabStrip.setDividerDrawable(t.getDrawable(R.styleable.SuperTabLayout_tabDivider));
        mTabStrip.setDividerPadding(t.getDimensionPixelSize(R.styleable.SuperTabLayout_tabDividerPadding, 0));
        mTabStrip.setShowDividers(t.getInt(R.styleable.SuperTabLayout_tabShowDividers, 0));
        //------add end  ------
        t.recycle();

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SuperTabLayout,
                defStyleAttr, R.style.Widget_Design_TabLayout);


        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.SuperTabLayout_tabIndicatorHeight, 0));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.SuperTabLayout_tabIndicatorColor, 0));
        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.SuperTabLayout_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.SuperTabLayout_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.SuperTabLayout_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.SuperTabLayout_tabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.SuperTabLayout_tabPaddingBottom,
                mTabPaddingBottom);

        mTabTextAppearance = a.getResourceId(R.styleable.SuperTabLayout_tabTextAppearance,
                android.R.style.TextAppearance);

        DEFAULT_GAP_TEXT_ICON = a.getDimensionPixelSize(R.styleable.SuperTabLayout_tabTextIconGap,
        ...
        if (a.hasValue(R.styleable.SuperTabLayout_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.SuperTabLayout_tabTextColor);
        }

        if (a.hasValue(R.styleable.SuperTabLayout_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.SuperTabLayout_tabSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors == null ? selected : mTabTextColors.getDefaultColor(), selected);
        }

        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.SuperTabLayout_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.SuperTabLayout_tabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.SuperTabLayout_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.SuperTabLayout_tabContentStart, 0);
        mMode = a.getInt(R.styleable.SuperTabLayout_tabMode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.SuperTabLayout_tabGravity, GRAVITY_FILL);
        a.recycle();
    ...
    //------add当需要与AutoLayout配合时使用 start------
    @NonNull
    protected SlidingTabStrip createSlidingTabStrip(Context context) {
     ...
     * @attr ref android.support.design.R.styleable#SuperTabLayout_tabIndicatorColor
     */
    ...
    /**
     * Add a {@link SuperTabLayout.OnTabSelectedListener} that will be invoked when tab selection
     * changes.
     * <p>
    ...
    /**
     * Remove the given {@link SuperTabLayout.OnTabSelectedListener} that was previously added via
    ...
    /**
     * Remove all previously added {@link SuperTabLayout.OnTabSelectedListener}s.
     */
    ...
    /**
     * Returns the current mode used by this {@link SuperTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link SuperTabLayout} with a {@link ViewPager}.
     * <p>
    ...
    public void bindViewPager(@Nullable ViewPager viewPager) {
    ...
    public void setTabsFromPagerAdapter(@Nullable final PagerAdapter adapter) {
        setPagerAdapter(adapter, false, true);
    }
    ...
    private void setPagerAdapter(@Nullable final PagerAdapter adapter, final boolean addObserver) {
        setPagerAdapter(adapter, addObserver, true);
    }

    private void setPagerAdapter(@Nullable final PagerAdapter adapter, final boolean addObserver,
                                 boolean populateFromPagerAdapter) {
     ...
     * @hide
     */
    @IntDef(flag = true, value = {INDICATOR_WIDTH_WRAP, INDICATOR_WIDTH_MATCH, INDICATOR_WIDTH_EXACTLY})
    @Retention(RetentionPolicy.SOURCE)
    public @interface TabIndicatorWidthMode {
    }

    @IntDef(flag = true, value = {INDICATOR_HEIGHT_MATCH, INDICATOR_HEIGHT_WRAP, INDICATOR_HEIGHT_EXACTLY})
    @Retention(RetentionPolicy.SOURCE)
    public @interface TabIndicatorHeightMode {
    ...
    public static final class Tab {
        ...
        private SuperTabLayout mParent;
     ...
     * necessary calls back to the provided {@link SuperTabLayout} so that the tab position is
     * kept in sync.
     * <p/>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<SuperTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(SuperTabLayout superTabLayout) {
            mTabLayoutRef = new WeakReference<>(superTabLayout);
        }
        ...
        public void onPageScrolled(int position, float positionOffset,
            ...
            final SuperTabLayout superTabLayout = mTabLayoutRef.get();
            if (superTabLayout != null) {
                ...
                superTabLayout.setScrollPosition(position, positionOffset, updateText, updateIndicator);
            }
        }
        ...
        public void onPageSelected(int position) {
            final SuperTabLayout superTabLayout = mTabLayoutRef.get();
            if (superTabLayout != null && superTabLayout.getSelectedTabPosition() != position
                    && position < superTabLayout.getTabCount()) {
                // Select the tab, only updating the indicator if we're not being dragged/settled
                // (since onPageScrolled will handle that).
                final boolean updateIndicator = mScrollState == SCROLL_STATE_IDLE
                        || (mScrollState == SCROLL_STATE_SETTLING
                        && mPreviousScrollState == SCROLL_STATE_IDLE);
                superTabLayout.selectTab(superTabLayout.getTabAt(position), updateIndicator);
            }
        }
    ...
    /**
     * A {@link SuperTabLayout.OnTabSelectedListener} class which contains the necessary calls back
     * to the provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements SuperTabLayout.OnTabSelectedListener {
    ...
    protected class TabView extends LinearLayout implements OnLongClickListener {
        ...
        public int getContentLeft() {
            if (mCustomView != null) {
                return mCustomView.getLeft() + getLeft();
            } else if (mTab != null && mTab.getIcon() != null && !TextUtils.isEmpty(mTab.getText())) {
                if (mTextView.getWidth() > mIconView.getWidth()) {
                    return mTextView.getLeft() + getLeft();
                } else {
                    return mIconView.getLeft() + getLeft();
                }
            } else if (mTab != null && !TextUtils.isEmpty(mTab.getText())) {
                return mTextView.getLeft() + getLeft();
            } else if (mTab != null && mTab.getIcon() != null) {
                return mIconView.getLeft() + getLeft();
            } else {
                return getLeft();
            }
        ...
        public int getContentTop() {
            if (mCustomView != null) {
                return mCustomView.getTop() - mCustomView.getPaddingTop();
            } else if (mTab != null && mTab.getIcon() != null && mIconView != null) {
                return mIconView.getTop() - mIconView.getPaddingTop();
            } else if (mTab != null && !TextUtils.isEmpty(mTab.getText()) && mTextView != null) {
                return mTextView.getTop() - mTextView.getPaddingTop();
            } else {
        ...
        public int getContentRight() {
            if (mCustomView != null) {
                return mCustomView.getRight() + getLeft();
            } else if (mTab != null && mTab.getIcon() != null && !TextUtils.isEmpty(mTab.getText())) {
                if (mTextView.getWidth() > mIconView.getWidth()) {
                    return mTextView.getRight() + getLeft();
                } else {
                    return mIconView.getRight() + getLeft();
                }
            } else if (mTab != null && !TextUtils.isEmpty(mTab.getText())) {
                return mTextView.getRight() + getLeft();
            } else if (mTab != null && mTab.getIcon() != null) {
        ...
        public int getContentBottom() {
            int bottom = 0;
            if (mCustomView != null) {
                bottom = mCustomView.getBottom() - mCustomView.getPaddingBottom();
            } else if (mTab != null && mTab.getIcon() != null && mIconView != null) {
                bottom = mIconView.getBottom() - mIconView.getPaddingBottom();
            } else if (mTab != null && !TextUtils.isEmpty(mTab.getText()) &&
                    mTextView != null) {
                bottom = mTextView.getBottom() - mTextView.getPaddingBottom();
            }
            return bottom == 0 ? getBottom() - getPaddingBottom() : bottom;
        }
...
//                    bottomMargin = dpToPx(DEFAULT_GAP_TEXT_ICON);
                    bottomMargin = DEFAULT_GAP_TEXT_ICON;
                    //------modify end--------
                }
    ...
    protected class SlidingTabStrip extends LinearLayout {
        ...
        private int indicatorHeightMode;
        ...
        void setIndicatorHeightMode(@TabIndicatorHeightMode int mode) {
            if (indicatorHeightMode != mode) {
                indicatorHeightMode = mode;
                ViewCompat.postInvalidateOnAnimation(this);
            }
        }
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                if (indicatorWidthMode == INDICATOR_WIDTH_MATCH) {
                ...
                } else if (indicatorWidthMode == INDICATOR_WIDTH_WRAP) {
                    left = ((TabView) selectedTitle).getContentLeft();
                    right = ((TabView) selectedTitle).getContentRight();
                    if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                        // Draw the selection partway between the tabs
                        View nextTitle = getChildAt(mSelectedPosition + 1);
                        left = (int) (mSelectionOffset * (((TabView) nextTitle).getContentLeft())
                                + (1.0f - mSelectionOffset) * left);
                        right = (int) (mSelectionOffset * (((TabView) nextTitle).getContentRight())
                                + (1.0f - mSelectionOffset) * right);
                    }
                } else {
                    int titleLeft = selectedTitle.getLeft();
                    int titleRight = selectedTitle.getRight();
                    int middle = (titleRight - titleLeft) / 2;
                    int indicatorWidth = mTabIndicatorWidth / 2;
                    left = titleLeft + middle - indicatorWidth;
                    right = left + mTabIndicatorWidth;
                    //动画时
                    if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                        // Draw the selection partway between the tabs
                        View nextTitle = getChildAt(mSelectedPosition + 1);
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            if (indicatorWidthMode == INDICATOR_WIDTH_MATCH) {
                ...
                targetLeft = tabView.getContentLeft();
                targetRight = tabView.getContentRight();
            } else {
                int targetMiddle = (targetView.getRight() - targetView.getLeft()) / 2;
                int indicatorWidth = mTabIndicatorWidth / 2;
                targetLeft = targetView.getLeft() + targetMiddle - indicatorWidth;
                targetRight = targetLeft + mTabIndicatorWidth;
            }
            final int startLeft;
        ...
        public void draw(Canvas canvas) {
            /**attention please:tabBackground和indicator的height wrap相冲突，
             * 如果height为wrap同时又要设置背景时请以tabLayout的background来替代，以达到相同的效果
             **/
            // Thick colored underline below the current selection
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                int indicatorTop;
                int indicatorBottom;
                if (indicatorHeightMode == INDICATOR_HEIGHT_MATCH) {
                    indicatorTop = getTop() - getPaddingTop();
                    indicatorBottom = getBottom() - getPaddingBottom();
                } else if (indicatorHeightMode == INDICATOR_HEIGHT_WRAP) {
                    TabView targetView = (TabView) getChildAt(mSelectedPosition);
                    indicatorTop = targetView.getContentTop();
                    indicatorBottom = targetView.getContentBottom();
                } else {
                    indicatorTop = getHeight() - this.mSelectedIndicatorHeight;
                    indicatorBottom = getHeight();
                }
                rectF.left = mIndicatorLeft;
                rectF.top = indicatorTop;
                ...
                if (cornerRadius != 0) {
                    ...
                    canvas.drawRect(rectF, mSelectedIndicatorPaint);
                }
            }
            //修改draw的顺序，先画indicator，再通过super.draw（）来画child
            super.draw(canvas);
        }
    }
